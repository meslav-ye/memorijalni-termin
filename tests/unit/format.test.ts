...
describe("formatMatchDateTime", () => {
  it("ispisuje dan, datum i vrijeme u zagrebackoj zoni (ljetno vrijeme)", () => {
    ...
    expect(formatMatchDateTime("2026-09-08T18:00:00.000Z")).toBe("uto 08.09.2026. u 20:00");
  });
  ...
  it("ispravno racuna i zimsko vrijeme", () => {
    ...
    expect(formatMatchDateTime("2026-12-08T19:00:00.000Z")).toBe("uto 08.12.2026. u 20:00");
  });

  it("koristi hrvatske kratice dana", () => {
    // 2026-09-06 je nedjelja.
    expect(formatMatchDateTime("2026-09-06T10:00:00.000Z")).toMatch(/^ned /);
  });
});
...
describe("zagrebUIso", () => {
  ...
  it("ono sto korisnik upise, to i vidi natrag", () => {
    ...
    for (const datum of ["2026-01-15", "2026-06-15", "2026-09-08", "2026-12-08"]) {
      expect(formatMatchDateTime(zagrebUIso(datum, "20:00"))).toContain("u 20:00");
    }
  });
